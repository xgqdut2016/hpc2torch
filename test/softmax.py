...
def test(test_shape, test_axis, device):
    ...
    if device == "cuda":
        ...
        torch_softmax_time = performance.AscendProfile((torch.softmax, (Q, test_axis)))  # 以毫秒为单位
        lib.softmax_aclnn.argtypes = [
            ctypes.POINTER(ctypes.c_void_p),
            ctypes.POINTER(ctypes.c_void_p),
            ctypes.c_int,
            ctypes.c_int,
        ...
        custom_softmax_time = performance.AscendProfile((lib.softmax_aclnn, (input_ptr, output_ptr, ndim, test_axis, ctypes_array, byteSize)))
