...
def test(
    ...
    if test_dtype == torch.float16 or test_dtype == torch.bfloat16:
        ...
        atol = 3e-3; rtol = 5e-3
    print(
        f"Testing Per Token Dequant Fp8 on {device} with x_shape:{x_shape}, symmetric:{symmetric}, dtype:{test_dtype}"
    )
    
    x = torch.zeros(x_shape, device=device, dtype=test_dtype, requires_grad=False)
    num_tokens, hidden_dim = x_shape
    x_scale = torch.randn((num_tokens, 1), device=device, dtype=torch.float32, requires_grad=False)
    x_packed = torch.rand(x_shape, device=device, dtype=torch.float32, requires_grad=False).to(dtype=torch.float8_e4m3fn)

    ans = dequantize_per_token_sym(x_packed, x_scale, test_dtype)
    ...
    if device == "cuda":
        torch_quant_linear_time = performance.CudaProfile((dequantize_per_token_sym, (x_packed, x_scale, test_dtype))) 
        lib.PerTokenDequantF8_nv.argtypes = [
        ...
        performance.CudaProfile((lib.PerTokenDequantF8_nv, (
