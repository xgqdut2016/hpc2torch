                                 ...
                                 int64_t num_bits, bool is_a_8bit)
{
  ...
  host::RuntimeCheck(size_k % marlin::tile_k_size == 0, "size_k = ", size_k,
                     " is not divisible by tile_k_size = ", marlin::tile_k_size);
  host::RuntimeCheck(size_n % marlin::tile_n_size == 0, "size_n = ", size_n,
                     " is not divisible by tile_n_size = ", marlin::tile_n_size);

  host::RuntimeCheck(num_bits == 4 || num_bits == 8,
                     "num_bits must be 4 or 8. Got = ", num_bits);
  int const pack_factor = 32 / num_bits;

  // Verify B
  host::RuntimeCheck((size_k / pack_factor) == b_q_weight.size(0),
                     "Shape mismatch: b_q_weight.size(0) = ", b_q_weight.size(0),
                     ", size_k = ", size_k, ", pack_factor = ", pack_factor);
  host::RuntimeCheck(b_q_weight.size(1) == size_n,
                     "b_q_weight.size(1) = ", b_q_weight.size(1),
                     " is not size_n = ", size_n);

  // Verify device and strides
  host::RuntimeCheck(b_q_weight.device().is_cuda(), "b_q_weight is not on GPU");
  host::RuntimeCheck(b_q_weight.is_contiguous(), "b_q_weight is not contiguous");
  host::RuntimeCheck(b_q_weight.dtype() == at::kInt, "b_q_weight type is not kInt");

  host::RuntimeCheck(perm.device().is_cuda(), "perm is not on GPU");
  host::RuntimeCheck(perm.is_contiguous(), "perm is not contiguous");
  host::RuntimeCheck(perm.dtype() == at::kInt, "perm type is not at::kInt");
  ...
  {
    host::RuntimeCheck(false, "Unsupported repack config: num_bits = ", num_bits,
