...
void GptqGemmKernel(void *c, const void *a, const void *b,
                    const void *b_scales, const void *b_zeros, const void *b_g_idx,
                    int M, int K, int N, int num_groups, int b_size_0,
                    bool use_exllama, int64_t bit)
{

    half *temp_dq;
    cudaMalloc((void **)&temp_dq, b_size_0 * 32 / bit * N * sizeof(half));
...
extern "C" void gptq_gemm_nv(void *c, const void *a, const void *b,
                             const void *b_scales, const void *b_zeros, const void *b_g_idx,
                             int M, int K, int N, int num_groups, int b_size_0,
                             bool use_exllama, int bit)
{
    GptqGemmKernel(c, a, b,
                   b_scales, b_zeros, b_g_idx,
                   M, K, N, num_groups, b_size_0,
                   use_exllama, static_cast<int64_t>(bit));
}